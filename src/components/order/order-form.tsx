    ...
    <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-deep-blue/65">{stepMeta[step].description}</p>
        </div>
              ...
              <div className="space-y-3 rounded-2xl bg-sand/50 p-4">
                <label className="flex items-start gap-3">
                  <input
                    ...
                    className="mt-1 h-4 w-4 rounded border-deep-blue/30 text-ocean focus:ring-ocean"
                  />
                  <span className="text-sm text-deep-blue/85">
        ...
        <div className="flex flex-col-reverse items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
          <button
            ...
            disabled={step === 1}
            className={cn(
              "inline-flex h-12 items-center justify-center gap-1.5 rounded-xl border border-deep-blue/15 px-5 font-semibold text-deep-blue transition",
              step === 1 ? "opacity-40" : "hover:border-deep-blue/40",
            )}
          >
          ...
          {step < 3 ? (
            <button
              type="button"
              onClick={goNext}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-deep-blue px-6 font-semibold text-white shadow-soft transition hover:bg-deep-blue/90"
            >
            ...
            <button
              ...
              className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-coral px-6 font-semibold text-white shadow-soft transition hover:bg-coral/90 disabled:opacity-70"
            >
              <MessageCircle className="h-4 w-4" /> Siparişi Gönder — 5 Dakikada Arıyoruz
            </button>
          )}
        </div>
      </form>
      ...
      <aside className="space-y-6 lg:sticky lg:top-24">
        <div className="rounded-2xl border border-deep-blue/10 bg-white p-6 shadow-soft">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ocean">
            Acelesi var
          </p>
          ...
          <Link
            href={telLink()}
            className="mt-4 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-deep-blue font-semibold text-white"
          >
            <Phone className="h-4 w-4" /> {siteConfig.contact.phone}
          </Link>
          <Link
            href={waLink(`Merhaba, ${siteConfig.shortName} için sipariş vermek istiyorum.`)}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-2 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-whatsapp font-semibold text-white"
          >
            <MessageCircle className="h-4 w-4" /> WhatsApp&apos;tan Yaz
          </Link>
        </div>

        <div className="rounded-2xl bg-sand/60 p-6 text-sm text-deep-blue/75">
          ...
          <ol className="mt-3 space-y-2 list-decimal pl-5 text-sm">
            <li>WhatsApp&apos;tan size formatlanmış sipariş özetinizi gönderiyoruz.</li>
        ...
        <div key={n} className="flex flex-1 items-center gap-3">
          <span
            className={cn(
              ...
              step >= n
                ? "bg-ocean text-white"
                : "border border-deep-blue/15 text-deep-blue/40",
            )}
          >
          ...
          {n < 3 && (
            <span
              className={cn(
                ...
                step > n ? "bg-ocean" : "bg-deep-blue/10",
              )}
            />
          )}
        </div>
...
function Field({
  ...
  return (
    <label className="block space-y-2">
      <span className="block text-sm font-semibold text-deep-blue">{label}</span>
      {children}
      {hint && !error && (
        <span className="block text-xs text-deep-blue/55">{hint}</span>
      )}
      {error && <span className="block text-xs text-coral">{error}</span>}
    </label>
  );
}
    ...
    <button
      ...
      className={cn(
        ...
        selected
          ...
          : "border-deep-blue/15 bg-white text-deep-blue/85 hover:border-ocean/40",
      )}
    >
      {label}
    </button>
